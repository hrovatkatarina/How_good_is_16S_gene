...
df <- cbind('Base_Position' = base_position, entropy)
df <- as.data.frame(df)
...
plt <- ggplot(df)
plt <- plt + geom_line(aes(x=Base_Position, y=entropy), lwd=1) +
  labs(x = "Position along 16S gene", y = "Standardized entropy") +
  theme(plot.title = element_text(size = 15), 
        axis.title.x = element_text(size = 12), 
        axis.title.y = element_text(size = 12))
plt <- plt + theme_bw() + ylim(-0.23, 1.06) + scale_y_continuous(breaks = c(0, 0.25, 0.5, 0.75, 1),
...
ggsave("Entropy_plot.png", plot = plt, width = 8, height = 5)