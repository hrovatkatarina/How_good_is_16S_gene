...
for record in SeqIO.parse(infile, "fasta"):
    ...
    genera.add(genus)
    output_file = f"{genus}_new.fasta"
...
#     # unique: uniqe elements in the column counts: the number of times each unique item appears
#         freq = counts / len(column)
#         entropy.append(-np.sum(freq * np.log2(freq)))  # sum entropy for each uniqe nc in the column
#
...
# entropy_df.to_csv('Std_entropy.csv', index=True, header=False)
...
#
# entropy_df_s = pd.DataFrame(entropy_data_s)
# entropy_df_s.to_csv('Smooth_std_entropy.csv', index=True, header=False)
#print(entropy_df_s.head())
